...
//   }
  ProductonChange($event){
   ...
   let checkcond=false;
    ...
    this.catid=$event.toString();
    if(this.selectcat!=""){
      checkcond = this.selectcat.includes(this.catid);
    ...
    //this.selectcat=this.catid.split(',');
  ...
  getProducts(catid) {
    ...
    this.bookService.getProductsbycatid(catid).subscribe((Response) => {
      ...
      this.getProductResponse=Response;
       ...
       for(var ii=0;ii<this.searchresponse.length;ii++){
        this.searchresponse[ii].ActualPrice=this.searchresponse[ii].Price;
        this.searchresponse[ii].ItemDiscountAmount=0;
     ...
     let hhi=false;
      this.searchresponse.sort((a,b) => a.Name > b.Name ? 1 : -1)
...
for (let person of this.orginalProduct1) {
  ...
  // let todaydate=this.dateDiffInDays(new Date(), new Date('April 30, 2021 03:24:00'));
 //  if(todaydate>0){
  let mmcart=[];

  mmcart = this.cart.filter((cart) => {
    return (((cart.Amount) >0) && (cart.WeightCount) >0);
      ...
      if(mmcart.length>0){
        for (let person of mmcart) {
          this.cart.push(person);
        }
      }
  

      this.cart = Array.from(new Set(this.cart.map(a => a.ID)))
       .map(id => {
         return this.cart.find(a => a.ID === id)
       })
    ...
    if (this.cart.length > 0) {
      ...
      for (var o = 0; o < this.cart.length; o++) {
        ...
        this.cart[o]["flag"] = "";
          ...
          }else{ this.cart[o]["ItemDiscountAmount"] = 0;}
