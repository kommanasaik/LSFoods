...
/**
 * Generated class for the CredcustomerreportPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-credcustomerreport',
  templateUrl: 'credcustomerreport.html',
})
export class CredcustomerreportPage {
  ...
  */
    constructor(
      ...
      this.headerTitle="Credit Customer Reports";
  ...
  */
    ...
    gotoedit(cid){
      this.ordersList = this.products.filter((item) => {
        return (item.CustomerName==cid);
      })
      this.ordersList=this.ordersList[0].CustomerOrders;
      console.log(this.ordersList[0].CustomerOrders);
      this.hidden=true;
    }
    ProductonChange($event){
    
      this.customerID=$event.toString();
  ...
  */
  ...
  submitDate() {
    this.hidden=false;
      ...
      this.utils.getcreditcustomersreport(this.fromdate,this.todate,this.customerID).subscribe((Response) => {
        ...
        // this.products=this.products.CustomerOrders;
        }
        else{
